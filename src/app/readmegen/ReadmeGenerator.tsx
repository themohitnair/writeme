...
export function ReadmeGenerator({ installationId }: ReadmeGeneratorProps) {
    ...
    const generateReadme = useCallback(async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(
                `https://writeme-api.themohitnair.workers.dev/readmegen?owner=${encodeURIComponent(owner)}&repository=${encodeURIComponent(repository)}&branch=${encodeURIComponent(branchToUse)}&installation_id=${installationId}`
            )
    ...
    }, [owner, repository, branch, installationId])
